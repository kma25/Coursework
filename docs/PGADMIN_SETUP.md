...
После настройки рабочей базы запустите `MainApp` и выберите регистрацию.

Первый зарегистрированный пользователь в пустой базе автоматически получает роль `admin`. Это нужно, чтобы потом можно было создавать пользователей через команду:

```text
admin user create <логин> <пароль> [роль]
```

## 6. Где менять подключение в проекте

...
По умолчанию `CheckUpdatesOnStartup=true`, поэтому при запуске `MainApp` сразу выводит результат проверки обновлений: найдена новая версия, обновлений нет или GitHub временно ограничил запросы. Если для демонстрации нужно убрать сетевую проверку при старте, можно поставить `false` и запускать проверку вручную командой `update check`.
